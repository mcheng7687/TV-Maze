...
async function searchShows(query) {
  ...
  for (let series of response.data) {
    ...
    const info = {id: series.show.id, 
                name: series.show.name, 
                summary: series.show.summary,
                image: !series.show.image ? null : series.show.image.medium};

    showArrays.push(info);
  }
      ...
      `<div class="col-md-6 col-lg-3 Show" data-show-id="${show.id}">
         <div class="card" data-show-id="${show.id}">
           <div class="card-body">
             ...
             <p class="card-text">${show.summary ? show.summary : 'No summary available'}</p>
...
$("#search-form").on("submit", async function handleSearch (evt) {
  ...
  $("#shows-list").on("click", async function (evt) {
    ...
    let episodesList = await getEpisodes(evt.target.id);
...
async function getEpisodes(id) {
  ...
  const episodesList = await axios.get(`https://api.tvmaze.com/shows/${id}/episodes`);
  const episodesInfo = [];

  for (let episode of episodesList.data) {
